...
class CarEnv:
    ...
    def step(self, action):
        ...
        for i in range(0, self.step_frames):
            ...
            if self.sensor.front_crash():  # or self.sensor.rx_back_crash() or self.sensor.lx_back_crash():
                while self.sensor.front_crash():  # or self.sensor.rx_back_crash() or self.sensor.lx_back_crash():
                    self.car_to_safe()
                reward = -1
                ...
                self.car_stop_count = 0
                self.prev_action = None
                self.state = self.state.stateByAddingScreen(self.camera.capture_as_rgb_array_bottom_half(),
                                                            self.frame_number)
